...
class TempTracker(object):
    ...
    temp=[]
    total, mode, min, max, count=0,0,0,0,0
    dic={}
    def insert(self, temperature):
        ...
        if(len(self.dic)==0):
            ...
            self.dic[temperature]=1

        elif(temperature in self.dic.keys()):
            if(self.min>temperature):
                self.min=temperature
            if(self.max<temperature):
                self.max=temperature
            self.dic[temperature]=self.dic[temperature]+1
            if(self.dic[temperature]>self.dic[self.mode]):
                self.mode = temperature
